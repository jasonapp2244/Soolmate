...
import 'package:soolmate_ui/screen/map_screen/map_screen.dart';
...
class ExploreScreenViewModel extends BaseViewModel {
  ...
  void prefSetting() async {
    ...
    suggestPhotoTapStatus =
        await PrefService.getInt(PrefConst.suggestPhotoTap) ?? 0;
  ...
  FutureOr<bool> onSwipe(
    ...
    if (userList.length - 3 == currentProfileIndex ||
        userList.length - 1 == currentProfileIndex) {
      exploreScreenApiCall();
    }
    return true;
  }
  ...
  void onLikeDislikeTap(bool isLikeAction) {
    ...
    ApiProvider()
        .updateLikedProfile(userList[currentProfileIndex].id)
        .then((value) {
