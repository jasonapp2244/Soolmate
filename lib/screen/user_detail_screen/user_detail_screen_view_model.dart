...
import 'package:soolmate_ui/model/chat_and_live_stream/chat.dart';
import 'package:soolmate_ui/model/chat_and_live_stream/live_stream.dart';
import 'package:soolmate_ui/model/setting.dart';
import 'package:soolmate_ui/model/user/follow_user.dart';
import 'package:soolmate_ui/model/user/registration_user.dart';
import 'package:soolmate_ui/screen/chat_screen/chat_screen.dart';
import 'package:soolmate_ui/screen/edit_profile_screen/edit_profile_screen.dart';
import 'package:soolmate_ui/screen/person_streaming_screen/person_streaming_screen.dart';
import 'package:soolmate_ui/screen/post_screen/post_screen.dart';
import 'package:soolmate_ui/screen/user_report_screen/report_sheet.dart';
import 'package:soolmate_ui/service/pref_service.dart';
import 'package:soolmate_ui/utils/asset_res.dart';
import 'package:soolmate_ui/utils/firebase_res.dart';
...
class UserDetailScreenViewModel extends BaseViewModel {
  ...
  UserDetailScreenViewModel(
      {this.userId, this.otherUserData, this.onUpdateUser});
  ...
  Future<void> userProfileApiCall() async {
    ...
    try {
      final profileResponse =
          await ApiProvider().getProfile(userID: userId ?? otherUserData?.id);
  ...
  Future<void> registrationUserApiCall() async {
    ...
    await ApiProvider().getProfile(userID: PrefService.userId).then((value) {
      ...
      blockUnBlock =
          value?.data?.blockedUsers?.contains('${otherUserData?.id}') == true
              ? S.current.unBlock
              : S.current.block;
      save =
          value?.data?.savedProfile?.contains('${otherUserData?.id}') ?? false;
      notifyListeners();
  ...
  void userDetailApiCall() async {
    ...
    if (_isValidCoordinate(latitude) &&
        _isValidCoordinate(otherUserData?.latitude)) {
  ...
  double calculateDistance({
    ...
    return 12742 * asin(sqrt(a)); // Earth's diameter in km
  }
  ...
  void onChatBtnTap() {
    ...
    Conversation conversation = Conversation(
      block: myUserData?.blockedUsers?.contains('${otherUserData?.id}') == true
          ? true
          : false,
      blockFromOther:
          otherUserData?.blockedUsers?.contains('${myUserData?.id}') == true
              ? true
              : false,
      conversationId: CommonFun.getConversationID(
          myId: myUserData?.id, otherUserId: otherUserData?.id),
      deletedId: '',
      ...
      FlutterBranchSdk.getShortUrl(buo: buo, linkProperties: lp).then((value) {
        if (value.success) {
          Share.share(value.result ?? '',
              subject: otherUserData?.fullname ?? '');
        } else {
  ...
  void onReportTap() {
    Get.bottomSheet(
      ReportSheet(
          ...
          fullName: otherUserData?.fullname,
          profileImage:
              CommonFun.getProfileImage(images: otherUserData?.images),
          age: otherUserData?.age,
  ...
  void onEditBtnClick() {
    Get.to<RegistrationUserData>(
        () => EditProfileScreen(userData: otherUserData))?.then((value) {
      if (value != null) {
